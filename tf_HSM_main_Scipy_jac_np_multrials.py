...
#CUDA_VISIBLE_DEVICES=3 python tf_HSM_main_Scipy_jac_np_multrials.py REGION=1 RESTART_TRIAL=1 SEED=18 ITERATIONS=1 LR=1e-3 NUM_LGN=9 HLSR=0.2 | tee Seed_LOG/HSM_Tensorflow_region1_seed18_trial1.txt
# CUDA_VISIBLE_DEVICES=2 python tf_HSM_main_Scipy_jac_np_multrials.py REGION=1 RESTART_TRIAL=1 SEED=19 ITERATIONS=1 LR=1e-3 NUM_LGN=9 HLSR=0.2 | tee Seed_LOG/HSM_Tensorflow_region1_seed19_trial1.txt
...
def main():
    ...
    summary_dir = os.path.join(SUMMARY_DIR+"AntolikRegion%s_%s_MaxIter%g_itr%g_SEED%g_trial%g_%s"%(region_num,Code, MAXITER,iterations,SEED,runID, dt_stamp))
    summary_writer = tf.summary.FileWriter(summary_dir, sess.graph)


    loss_list, activation_summary_lgn, activation_summary_l1, yhat_std, MSE_list, corr_list = [], [], [], [], [],[]
    
    summary_fname = "HSMtensorflow_region"+region_num+"_seed%g_trial%g"%(SEED,RESTART_TRIAL)
