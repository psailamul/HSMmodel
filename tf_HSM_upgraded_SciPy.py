...
class tf_HSM():
    ...
    def construct_free_params(self):
      # LGN initialization
      self.lgn_x = tf.get_variable(name="x_pos", shape=self.num_lgn, initializer=self.LGN_init, trainable=self.lgn_trainable);self.bounds['x_pos']=(0,31) # 0-31
      self.lgn_y = tf.get_variable(name="y_pos", shape=self.num_lgn, initializer=self.LGN_init, trainable=self.lgn_trainable);self.bounds['y_pos']=(0,31)  # 0-31
      self.lgn_sc = tf.get_variable(name="size_center", shape=self.num_lgn, initializer=self.LGN_sc_init, trainable=self.lgn_trainable) ; self.bounds['size_center']=(0.1,31)   #0.1 - 31
      self.lgn_ss = tf.get_variable(name="size_surround", shape=self.num_lgn, initializer=self.LGN_init, trainable=self.lgn_trainable) ; self.bounds['size_surround']=(0.0,31)  #0.0 - 31
      self.lgn_rc = tf.get_variable(name="center_weight", shape=self.num_lgn, initializer=self.LGN_init, trainable=self.lgn_trainable) ; self.bounds['center_weight']=(0.0,10.0)   #0-10
      self.lgn_rs = tf.get_variable(name="surround_weight", shape=self.num_lgn, initializer=self.LGN_init, trainable=self.lgn_trainable) ; self.bounds['surround_weight']=(0.0,10.0)  #0-10
