...
def saveResults(HSM,suppl_params,K=None,errors=None,corr=None,prefix='HSMfit'): 
    """
    Saves HSM fitting results and meta-parameters
    """
    
    filenames=[]
   
    # Save K (HSM parameters)   
    if not(K in [None,[]]):
        saveMat(K,prefix+'_K')
        filenames.append(prefix+'_K')
    
    # Save meta-parameters
    meta_params = dict(HSM.get_param_values())
    suppl_params['stimsize']=HSM.size
...
def restoreHSM(checkpointname,training_inputs,training_set,update_mp={}):
    """
    Reloads meta-parameters of a checkpointed fit, updates some of them using update_mp, then uses them and the provided data to recreate a HSM object
    ...
    """
    # Load and update meta-parameters, use them to recreate the HSM object
    meta_params, suppl_params = loadParams(checkpointname+'_metaparams')
    meta_params.update(update_mp)
    HSM=HSM(training_inputs,training_set,**meta_params) 
