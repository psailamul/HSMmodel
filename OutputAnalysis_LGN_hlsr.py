...
LGN_list = [5,9,15]; hlsr_list=[0.1,0.2,0.3]
...
for lgn,hlsr in zip(LGN_list,hlsr_list):
	print "=================== LGN = %d , hlsr = %.1f =================="%(lgn,hlsr)
	[Ks,hsm] = np.load('out_Allen_unique_lgn%d_hlsr%d.npy'%(lgn,hlsr*10))
	...
	print "MAX,MIN Correlation between predicted response and recorded response(Training Set): %.2f, %.2f " %(np.max(corr),np.min(corr))


	vld_corr = computeCorr(pred_response,vld_set)
	print "MAX,MIN Correlation between predicted response and recorded response(Test Set): %.2f, %.2f " %(np.max(vld_corr),np.min(vld_corr))
	...
	plt.plot(train_input[:,imax],'-ok')
	plt.plot(response[:,imax],'--or')
	plt.title('Cell#%d has max corr of %f'%(imax+1,np.max(corr)))
	plt.show()

	imin = np.argmin(orr) # note : actually have to combine neurons in all regions

	plt.plot(train_input[:,imin],'-ok')
	plt.plot(response[:,imin],'--or')
